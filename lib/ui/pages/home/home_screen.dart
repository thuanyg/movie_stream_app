...
import 'package:movie_stream/ui/pages/home/components/all_movie_latest.dart';
...
import 'components/all_movie_genre.dart';
...
class _HomeScreenState extends State<HomeScreen>
  ...
  late Future<List<Item>?> _moviesByCategoryFuture;
  ...
  Future<void> _loadData() async {
    ...
    _moviesByCategoryFuture =
        provider.getMoviesByCategory(categories.keys.first, 1);
  }
                      ...
                      Padding(
                        ...
                        child: Column(children: [
                          ...
                          Category(
                            ...
                            onSeeAll: () {
                              provider.resetData();
                              provider.getListLatestMovie(1);
                              showModalBottomSheet(
                                  backgroundColor: AppColors.backgroundColor,
                                  context: context,
                                  isScrollControlled: true,
                                  builder: (context) {
                                    return AllLatestMovies(
                          ...
                          Category(
                            ...
                            onSeeAll: () async {
                              showBottomSheet(PHIM_BO, "Phim bộ ");
                              provider.resetData();
                              provider.getMoviesByGenre(PHIM_BO, 1);
                            },
                          ),

                          _buildListMovie(_seriesMoviesFuture),

                          // Odd films
                          Category(
                            categoryName: "Phim lẻ",
                            onSeeAll: () async {
                              showBottomSheet(PHIM_LE, "Phim lẻ");
                              provider.resetData();
                              provider.getMoviesByGenre(PHIM_LE, 1);
                            },
                          ),

                          _buildListMovie(_oddMoviesFuture),
                  ...
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      return FutureBuilder(
                        future: _moviesByCategoryFuture,
                    ...
                    childCount: LIMIT_INITIAL, // Number of items in the grid
                  ),
                ),
              ],
            ),
  ...
  showBottomSheet(String genre, String title) {
    showModalBottomSheet(
        backgroundColor: AppColors.backgroundColor,
        context: context,
        isScrollControlled: true,
  ...
  Widget _buildListMovie(Future<List<Item>> future) {
    final Size size = MediaQuery.of(context).size;
    return SizedBox(
      height: size.height / 3.7,
      child: FutureBuilder(
